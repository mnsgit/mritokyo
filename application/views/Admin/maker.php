                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <div class="modal-dialog modal-md">
                            <form role="form" method="post" class="form-horizontal" id="frmRegistration" name="frmRegistration" action="<?php echo base_url()?>admin/maker/post">
                            <input type="hidden" id="id" name="id" value="" />
                            <div class="modal-content">
                                <div class="modal-header">
                                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">
                                        ×</button>
                                    <h4 class="modal-title" id="myModalLabel"> Add Maker </h4>
                                </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-10">
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-7">
                                                    <input type="text" name="name" class="form-control" required id="name" placeholder="Enter maker name" />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
<script>
	...
	function callDML(id){
		...
		if ( 'undefined' != typeof(id) && id > 0 ) {
			var url = '<?php echo base_url() ?>admin/maker/edit/'+id;
			$.getJSON(url, function(obj) {
				$('#id').val(obj.id);
				$('#name').val(obj.name);
				$('#myModalLabel').html("Update Maker");
				$("#dmlModal").modal('show');
	...
	function remove(id) 
	{
		if (confirm('Are you sure you want to delete this?')) 
		{
			if ( 'undefined' != typeof id ) {
				var url = '<?php echo base_url() ?>admin/maker/delete/'+id;
				$.get(url, function() {
					$('#row-'+id).remove();
				}).fail(function() { alert('Unable to fetch data, please try again later.') });
			} else {
